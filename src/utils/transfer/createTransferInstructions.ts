...
import { InvalidBufferLength, TokenAmountTooLarge } from '../../errors';
...
export const addSigners = (keys: AccountMeta[], ownerOrAuthority: PublicKey, multiSigners: Signer[]): AccountMeta[] => {
...
class TokenAmount extends BN {
    ...
    static fromBuffer(buffer: Buffer): TokenAmount {
        if (buffer.length !== 8) {
            throw new InvalidBufferLength(`Invalid buffer length: ${buffer.length}`);
        }
